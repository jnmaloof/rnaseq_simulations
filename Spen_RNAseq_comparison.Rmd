...
time.pl + ggtitle("Average run time, 10M reads, 2 CPUs") + ylab("time (sec)")
```

## compare run + count time

```{r, inculde=verbose,results='hide'}
count.times <- ls(pattern=".+counts\\.time")
count.time.data <- sapply(count.times, function(x) get(x)["elapsed"])
count.time.data #not worth worrying about
```
...
percent.map.pl + ggtitle("Perecent Mapped Reads") + ylab("Percent") + coord_cartesian(ylim=c(90,100))
...
## compare total number of genes with counts ratio < ... (or do as a step plot)
