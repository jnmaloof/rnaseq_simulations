...
lapply(reads, head)
reads$zero.map <- reads$gene %in% genes.zero.map
#reads.zero <- lapply(reads,function(x) x[reads$gene %in% genes.zero.map])
table(reads$flag,reads$zero.map)
table(reads$NH,reads$zero.map) #number of hits

reads.df <- with(reads,data.frame(flag,cigar,NH,HI,AS,nM,gene,zero.map),stringsAsFactors=FALSE)

reads.df$multimap <- reads.df$NH>1

ggplot(reads.df,aes(x=nM,color=multimap,fill=multimap)) + geom_density(alpha=.5)

ggplot(reads.df,aes(x=multimap,y=nM,color=multimap,fill=multimap)) + geom_violin()

ggplot(reads.df,aes(x=as.factor(NH),y=nM)) + geom_boxplot()

ggplot(reads.df[reads.df$NH<2,],aes(x=as.factor(NH),y=nM)) + geom_violin()

#TODO: What is the length of the predicted transcripts?

genes.zero.map.zero.hit <- unique(substr(reads$qname[reads$NH==0],13,47))
genes.zero.map.zero.hit <- genes.zero.map.zero.hit[genes.zero.map.zero.hit %in% genes.zero.map]

#why do these exist?  one hit but poor mapQ?  check to see if they are correct...
genes.zero.map.one.hit <- unique(substr(reads$qname[reads$NH==1],13,47))
genes.zero.map.one.hit <- genes.zero.map.one.hit[genes.zero.map.one.hit %in% genes.zero.map]

genes.zero.map.multi.hit <- unique(substr(reads$qname[reads$NH>1],13,47))
genes.zero.map.multi.hit <- genes.zero.map.multi.hit[genes.zero.map.multi.hit %in% genes.zero.map]
