...
```{r compare_function}
CompareCounts <- function(known.counts,mapped.counts, title= NULL, plot = TRUE, chrom.separate = TRUE,correlation=TRUE,return=TRUE) {
  count.comparison <- merge(known.counts,pen_mapped.counts,by.x="lyc_id",by.y="ID",suffixes=c(".known",".mapped"))
  ...
  if(plot & chrom.separate) {
    print(qplot(x=count.known,y=count.mapped,data=lyc_count_comparison) + 
            facet_wrap(~ chrom) + 
