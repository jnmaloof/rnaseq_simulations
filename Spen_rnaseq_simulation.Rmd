...
GetMappedCounts <- function(prefix,
  ...
  if(type=="STAR-htseq") {
    mapped_counts <- system(paste("htseq-count --format=bam --stranded=no --idattr=Parent",
                 file.path(dir,paste(prefix,"Aligned.sortedByCoord.out.bam",sep="")),
                 gff),
                 intern=TRUE)
    system(paste("ls -lh", file.path(dir,paste(prefix,"Aligned.sortedByCoord.out.bam",sep=""))))
