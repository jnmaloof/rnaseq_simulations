...
```{r eval=FALSE, engine='bash'}
cd spen_sim_1
STAR --parametersFiles ../STAR.params.1 --readFilesIn ./sample_01_renamed.fasta.gz --readFilesCommand zcat --outFileNamePrefix STAR_1 
...
```{r eval=FALSE, engine='bash'}
cd ../slyc_sim_1
STAR --parametersFiles ../STAR.params.3 --readFilesIn ./sample_01_renamed.fasta.gz --readFilesCommand zcat --outFileNamePrefix STAR_1 
samtools index STAR_1Aligned.sortedByCoord.out.bam
```
## Evaluate results
...
```{r}
pen_known_counts <- read.table(pipe("./CountGene.py spen_sim_1/sample_01.fasta.gz"),col.names=c("id","count"),stringsAsFactors = FALSE)
summary(pen_known_counts)
head(pen_known_counts)

lyc_known_counts <- read.table(pipe("./CountGene.py slyc_sim_1/sample_01.fasta.gz"),col.names=c("id","count"),stringsAsFactors = FALSE)
summary(lyc_known_counts)
head(lyc_known_counts)
```
...
system.time({
  reads <- readDNAStringSet("spen_sim_1/sample_01.fasta")
    pen_known_counts <- as.data.frame(
      table(
...
system.time({
  ...
  repeat {
    ...
    genes <- as.data.frame(
      ...
      responseName="count",
      stringsAsFactors = FALSE)
    matches <- genes$id %in% names(pen_known_counts)
    pen_known_counts[genes$id[!matches]] <- 0 #initiate an entry for new genes
    pen_known_counts[genes$id] <- unlist(pen_known_counts[genes$id]) + genes$count
...
```{r }
pen_mapped_counts <- read.delim("spen_sim_1/STAR_1ReadsPerGene.out.tab",header = FALSE, stringsAsFactors = FALSE)
head(pen_mapped_counts)
pen_mapped_counts <- pen_mapped_counts[,1:2] # 3 and 4 are directional counts which we don't care about
colnames(pen_mapped_counts) <- c("ID","count")
pen_mapped_counts$ID <- sub("gene:","",pen_mapped_counts$ID,fixed=TRUE)

lyc_mapped_counts <- read.delim("slyc_sim_1/STAR_1ReadsPerGene.out.tab",header = FALSE, stringsAsFactors = FALSE)
head(lyc_mapped_counts)
lyc_mapped_counts <- lyc_mapped_counts[,1:2] # 3 and 4 are directional counts which we don't care about
colnames(lyc_mapped_counts) <- c("ID","count")
lyc_mapped_counts$ID <- sub("gene:","",lyc_mapped_counts$ID,fixed=TRUE)
```

### Comparison
```{r}
#check that I can use substring
unique(nchar(pen_known_counts$id)) #all are the same length
pen_known_counts$lyc_id <- substr(pen_known_counts$id,18,33)
head(pen_known_counts)
pen_count_comparison <- merge(pen_known_counts,pen_mapped_counts,by.x="lyc_id",by.y="ID",suffixes=c(".known",".mapped"))
head(pen_count_comparison)
cor(pen_count_comparison$count.known,pen_count_comparison$count.mapped)
plot(pen_count_comparison$count.known,pen_count_comparison$count.mapped,main="S. pennellii")

unique(nchar(lyc_known_counts$id)) #all are the same length
lyc_known_counts$lyc_id <- substr(lyc_known_counts$id,18,33)
head(lyc_known_counts)
lyc_count_comparison <- merge(lyc_known_counts,lyc_mapped_counts,by.x="lyc_id",by.y="ID",suffixes=c(".known",".mapped"))
head(lyc_count_comparison)
cor(lyc_count_comparison$count.known,lyc_count_comparison$count.mapped)
plot(lyc_count_comparison$count.known,lyc_count_comparison$count.mapped,main="S. lycopersicum")
```
Lyc much better than Pen.  Bummer!

how much is chromosome zero?
```{r}
pen_count_comparison$chrom <- substr(pen_count_comparison$lyc_id,6,7)
head(pen_count_comparison)
library(ggplot2)
qplot(x=count.known,y=count.mapped,data=pen_count_comparison) + facet_wrap(~ chrom)

lyc_count_comparison$chrom <- substr(lyc_count_comparison$lyc_id,6,7)
head(lyc_count_comparison)
library(ggplot2)
qplot(x=count.known,y=count.mapped,data=lyc_count_comparison) + facet_wrap(~ chrom)
```
...
```{r}
head(pen_count_comparison[pen_count_comparison$chrom=="01" & pen_count_comparison$count.mapped==0,],n=20)
library(Rsamtools)
...
zero_map_genes <- pen_count_comparison$id[pen_count_comparison$count.known>0 & pen_count_comparison$count.mapped==0]
...
```{r eval=FALSE, engine='bash'}
cd spen_sim_1
STAR --parametersFiles ../STAR.params.2 --readFilesIn ./sample_01_renamed.fasta.gz --readFilesCommand zcat --outFileNamePrefix STAR_2 
samtools index STAR_2Aligned.sortedByCoord.out.bam
```

### Load in mapped counts
```{r }
pen_mapped_counts.2 <- read.delim("spen_sim_1/STAR_2ReadsPerGene.out.tab",header = FALSE, stringsAsFactors = FALSE)
head(pen_mapped_counts.2)
pen_mapped_counts.2 <- pen_mapped_counts.2[,1:2] # 3 and 4 are directional counts which we don't care about
colnames(pen_mapped_counts.2) <- c("ID","count")
pen_mapped_counts.2$ID <- sub("gene:","",pen_mapped_counts.2$ID,fixed=TRUE)
```
...
pen_known_counts$lyc_id <- substr(pen_known_counts$id,18,33)
head(pen_known_counts)
pen_count_comparison.2 <- merge(pen_known_counts,pen_mapped_counts.2,by.x="lyc_id",by.y="ID",suffixes=c(".known",".mapped"))
head(pen_count_comparison.2)
cor(pen_count_comparison.2$count.known,pen_count_comparison.2$count.mapped)
plot(pen_count_comparison.2$count.known,pen_count_comparison.2$count.mapped)
```
Not so good!

how much is chromosome zero?
```{r}
pen_count_comparison.2$chrom <- substr(pen_count_comparison.2$lyc_id,6,7)
head(pen_count_comparison.2)
library(ggplot2)
qplot(x=count.known,y=count.mapped,data=pen_count_comparison.2) + facet_wrap(~ chrom)
```
Not limited to chrom zero
...
Lets take a look at the bam file and extract reads from a few genes with 0 mapped reads.  Skip chromosome 0
```{r}
head(pen_count_comparison.2[pen_count_comparison.2$chrom=="01" & pen_count_comparison.2$count.mapped==0,],n=20)
library(Rsamtools)
...
lapply(reads, head)
#pull the reads that should have mapped out into new list
zero_map_genes <- pen_count_comparison.2$id[pen_count_comparison.2$count.known>0 & pen_count_comparison.2$count.mapped==0]
reads$gene <- unlist(strsplit(reads$qname,";"))[c(FALSE,TRUE,FALSE,FALSE)]
lapply(reads, head)
reads_zero <- lapply(reads,function(x) x[reads$gene %in% zero_map_genes])
lapply(reads_zero,head,20)
...
```{r eval=FALSE, engine='bash'}
cd spen_sim_1
STAR --parametersFiles ../STAR.params.3 --readFilesIn ./sample_01_renamed.fasta.gz --readFilesCommand zcat --outFileNamePrefix STAR_3 
samtools index STAR_3Aligned.sortedByCoord.out.bam
```

### Load in mapped counts
```{r }
pen_mapped_counts.3 <- read.delim("spen_sim_1/STAR_3ReadsPerGene.out.tab",header = FALSE, stringsAsFactors = FALSE)
head(pen_mapped_counts.3)
pen_mapped_counts.3 <- pen_mapped_counts.3[,1:2] # 3 and 4 are directional counts which we don't care about
colnames(pen_mapped_counts.3) <- c("ID","count")
pen_mapped_counts.3$ID <- sub("gene:","",pen_mapped_counts.3$ID,fixed=TRUE)
```
...
pen_known_counts$lyc_id <- substr(pen_known_counts$id,18,33)
head(pen_known_counts)
pen_count_comparison.3 <- merge(pen_known_counts,pen_mapped_counts.3,by.x="lyc_id",by.y="ID",suffixes=c(".known",".mapped"))
head(pen_count_comparison.3)
cor(pen_count_comparison.3$count.known,pen_count_comparison.3$count.mapped)
plot(pen_count_comparison.3$count.known,pen_count_comparison.3$count.mapped)
```
Not so good!

how much is chromosome zero?
```{r}
pen_count_comparison.3$chrom <- substr(pen_count_comparison.3$lyc_id,6,7)
head(pen_count_comparison.3)
library(ggplot2)
qplot(x=count.known,y=count.mapped,data=pen_count_comparison.3) + facet_wrap(~ chrom)
```
Not limited to chrom zero
...
Lets take a look at the bam file and extract reads from a few genes with 0 mapped reads.  Skip chromosome 0
```{r}
head(pen_count_comparison.3[pen_count_comparison.3$chrom=="01" & pen_count_comparison.3$count.mapped==0,],n=20)
library(Rsamtools)
reads <- scanBam(file="spen_sim_1/STAR_2Aligned.sortedByCoord.out.bam",
                 param = ScanBamParam(what=scanBamWhat(),
                                      tag=c("NH","HI","AS","nM")))[[1]]
...
lapply(reads, head)
#pull the reads that should have mapped out into new list
zero_map_genes <- pen_count_comparison.3$id[pen_count_comparison.3$count.known>0 & pen_count_comparison.3$count.mapped==0]
reads$gene <- unlist(strsplit(reads$qname,";"))[c(FALSE,TRUE,FALSE,FALSE)]
lapply(reads, head)
reads_zero <- lapply(reads,function(x) x[reads$gene %in% zero_map_genes])
lapply(reads_zero,head,20)
