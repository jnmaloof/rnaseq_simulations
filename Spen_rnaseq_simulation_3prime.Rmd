...
for(s in unique(subjectHits(pen.lyc.overlaps))) {
  ...
  tmp.contig.names <- names(pen.contig.gr.filter[queryHits(tmp.contigs)])
  ...
  system(paste("sed 's/>/>",gene.name,"|/' tmp.fa.cap.contigs tmp.fa.cap.singlets >> Spen_contigs_singletons_round2.fa"))
}
setwd("../")
```
