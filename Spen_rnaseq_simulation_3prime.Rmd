...
I want to compile all of the contigs that hit a gene into a single contig.  I should pay attention to strand and, ideally, order.  After looking at some of this I think that the best way is to do a cap3 assembly contigs if there are multiple contigs.  Because I already know that these map to one gene I can use relaxed parameters in cap3.

```{r combine_contigs_blat_genome, eval=FALSE}
# for each subject hit
# grab all query hits
# figure out strandedness
# reverse comp if needed and string together by order
...
for(s in unique(subjectHits(pen.lyc.overlaps))) {
  ...
  system("cap3 tmp.fa -o 16 -s 251 -z 1 -p 66  -j 31")
  system(paste("sed 's/>/>",gene.name,"|/' tmp.fa.cap.contigs tmp.fa.cap.singlets >> Spen_contigs_singletons_round2.fa",sep="")) #not quite right...should put all in one fasta.  Doesn't matter in this case.
}
...
```{r view_Assemblies2}
pen.contigs2 <- readDNAStringSet("Assemblies_Sneo_Spen/Spen_contigs_singletons_round2.fa")
names(pen.contigs2)
grep("Contig1",names(pen.contigs2),invert = T)
hist(q)
```
