...
head(heinz.gff)
three.prime.df <- rbindlist(lapply(heinz.gff$Name[heinz.gff$type=="gene"][1:100], function(gene) {
  ...
  #strand.numeric <- eval(parse(text=paste(tmp$strand[1],"1")))
...
three.prime.GR <- makeGRangesFromDataFrame(test)
export(three.prime,"test.gff3","~/Sequences/ref_genomes/tomato/ITAG2.4_Chromo2.5/ITAG2.4_three_prime_250_250.gff3")
```
...
pen.mapped.counts <- GetMappedCounts(prefix = "STAR_1", dir = "spen_threeprime_sim_1", type="STAR-featureCounts")
lyc.mapped.counts <- GetMappedCounts(prefix = "STAR_1", dir = "slyc_threeprime_sim_1", type="STAR-featureCounts")
```
