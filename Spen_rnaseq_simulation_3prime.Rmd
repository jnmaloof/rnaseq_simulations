...
head(heinz.gff)
test <- rbindlist(lapply(heinz.gff$Name[heinz.gff$type=="gene"][1:100], function(gene) {
  ...
  result <- rbindlist(list(tmp[tmp$type=="gene",][1,],
                  tmp[tmp$type=="mRNA",][1,],
                  tmp[tmp$type=="exon",][1,]) )
  result$start <- stop.pos - strand.numeric*250
  result$end <- stop.pos + strand.numeric*250
  as.data.frame(result)
}))
test.GR <- makeGRangesFromDataFrame(test)
```
