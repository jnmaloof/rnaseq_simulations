...
system.time(
  simulate_experiment(fasta="~/Sequences/ref_genomes/S.pen/Spenn-v2-cds-annot_orthlogs_only.fa",
                      ...
                      reads_per_transcript = round(rexp(n=length(pen.transcripts.small)) * width(pen.transcripts.small) * 
                                                     1e7/ sum(width(pen.transcripts.small) )),
                      paired=FALSE,
...
system.time(
  simulate_experiment(fasta="~/Sequences/ref_genomes/tomato/ITAG2.4_Chromo2.5/ITAG2.4_cds.pen.orthologs.only.fasta",
                      ...
                      reads_per_transcript = round(rexp(n=length(lyc.transcripts.small)) * width(lyc.transcripts.small) * 
                                                     1e7/ sum(width(lyc.transcripts.small) )),
                      paired=FALSE,
                      error_model="illumina5",
                      bias="rnaf"
  )
...
## Run it!
```{r RunStar_1, eval=TRUE}
RunStar(fasta="sample_01_renamed.fasta.gz",prefix = "STAR_1",dir="spen_sim_2",n=2)
RunStar(fasta="sample_01_renamed.fasta.gz",prefix = "STAR_1",dir="slyc_sim_2",n=2)
```
...
RunKallisto(fasta = "spen_sim_2/sample_01_renamed.fastq.gz", index = "ITAG2.4._cds.pen.orthlogsonly.kai", dir = "spen_sim_2/kallisto" )
```
...
pen.count.comparison.1 <- CompareCounts(pen.known.counts, pen.mapped.counts, chrom.separate=FALSE, title = "S. pen. kallisto 21 mapped to cDNA.  Default parameters.  Kallisto counting")
```
