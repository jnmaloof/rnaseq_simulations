...
# Mapping to GENOMIC reference with kallisto

## build index
only needs to be done once:
```{r, eval=FALSE, engine='bash'}
kallisto index -i S.lyc_Chrom2.50.kai ~/Sequences/ref_genomes/tomato/ITAG2.4_Chromo2.5/
```
But I wonder if different kmers would give different results
...
## run kallisto
```{r, eval=TRUE, engine='bash'}
RunKallisto(fasta = "spen_sim_1/sample_01_renamed.fastq.gz", index = "S.lyc_Chrom2.50.kai", dir = "spen_sim_1/kallisto_genomic" )
```

## Load in mapped counts
```{r get_mapped_counts_kallisto }
pen.mapped.counts <- GetMappedCounts(dir = "spen_sim_1/kallisto_genomic", type="featureCounts",bam="kallisto.bam")
```

## _S. pennellii_
Compare known to mapped counts  
```{r compare_spen1_kallisto, echo=FALSE, results='hide', fig.width=8, fig.height=5}
pen.count.comparison.1 <- CompareCounts(pen.known.counts, pen.mapped.counts, chrom.separate=FALSE, title = "S. pen. kallisto mapped to genome.  Default parameters.  FeatureCounts counting")
```
