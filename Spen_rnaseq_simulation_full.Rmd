...
```{r kallisto_kmer_summarize, echo=FALSE, fig.width=2, fig.height=2}
plot(x=unlist(lapply(results.combined,function(x) x$kmer)),y=unlist(lapply(results.combined, function(x) x$time["elapsed"])),type="b",ylab="time",xlab="kmer")

plot(x=unlist(lapply(results.combined,function(x) x$kmer)),y=unlist(lapply(results.combined, function(x) x$corr.all[1])),type="b",ylab="Pearson correlation",xlab="kmer",main="Pearson all genes")
